...
" POD starts with ^=<word> and ends with ^=cut or ⁼=cut "
...
if exists("perl_include_pod")
  ...
  if exists("perl_fold")
    syn region perlPOD start="^=[a-z]" end="^=cut\s*[\'\"]\?" contains=@Pod,@Spell,perlTodo keepend fold
    syn region perlPOD start="^=cut" end="^=cut\s*[\'\"]\?" contains=perlTodo keepend fold
  else
    syn region perlPOD start="^=[a-z]" end="^=cut\s*[\'\"]\?" contains=@Pod,@Spell,perlTodo keepend
    syn region perlPOD start="^=cut" end="^=cut\s*[\'\"]\?" contains=perlTodo keepend
  endif
else
  " Use only the bare minimum of rules
  if exists("perl_fold")
    syn region perlPOD start="^=[a-z]" end="^=cut\s*[\'\"]\?" fold
  else
    syn region perlPOD start="^=[a-z]" end="^=cut\s*[\'\"]\?"
  endif
endif
...
syn keyword perlDATA            __END__ __DATA__
...
if exists("perl_fold")
  ...
  if !exists("perl_nofold_subs")
    ...
    syn region perlSubFold start="^\<test\>.*\<sub\>.*[^};]$" end="^};\s*\(#.*\)\=$" transparent fold keepend
